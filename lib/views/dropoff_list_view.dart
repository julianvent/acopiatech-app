...
class DropoffListView extends StatelessWidget {
  ...
  const DropoffListView({super.key, required this.dropoffs, required this.length});

  @override
  Widget build(BuildContext context) {
    if (dropoffs.isEmpty) {
      return const Center(child: Text('No se han registrado centros de acopio.'));
    }

    return Column(
      spacing: 15,
      children: List.generate(length > dropoffs.length ? dropoffs.length : length, (
        index,
      ) {
        ...
        return GestureDetector(
          onTap: () {
            Navigator.push(
              context,
              MaterialPageRoute(builder: (_) => DropoffDetailsView(dropoff: dropoff)),
            );
          },
          child: Card(
            child: Padding(
              ...
              child: ListTile(
                ...
                subtitle: Column(
                  ...
                  children: [
                    Row(
                      spacing: 10,
                      children: [Icon(Icons.phone), CustomCardText(dropoff.phoneNumber)],
                    ),
                    Row(
                      spacing: 10,
                      children: [
